    ...
    >
      <div
        ...
        :style="{
				...
				height:childrenHeightArray[tabIndex]+'px'
			}"
      >
        <slot></slot>
      </div>
...
export default {
  ...
  mounted() {
    ...
    const childs = this.$slots.default.filter(node => node.tag === 'div')
    this.childrenHeightArray = []
    childs.forEach(child => {
      this.childrenHeightArray.push(child.elm.clientHeight)
    })
  },
  methods: {
    getAngle(dx, dy) {
