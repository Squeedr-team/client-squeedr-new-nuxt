<template>
  <ul class="child-panel" :class="{ 'pop-panel': !open && !isMobile }">
    <li
      v-for="(menu, idx) in childItem"
      :key="idx"
      :class="[isMainMenu ? 'menu-item' : 'sub-menu-item']"
    >
      <template v-if="!menu.children">
        <nuxt-link :to="menu.pathName">
          {{ menu.name }}
        </nuxt-link>
      </template>
      <template v-else>
        <template v-if="open || isMobile">
          <button
            :class="[
              opened === menu.id ? 'active' : '',
              isMainMenu ? 'menu-item-btn' : 'sub-menu-item-btn',
            ]"
