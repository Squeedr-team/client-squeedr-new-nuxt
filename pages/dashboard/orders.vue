    ...
    <div class="bg-white rounded-2xl sm:m-5 m-3 p-3 sm:p-5">
      ...
      >
        ...
        <template #cell(order)="{ item }">
          <div>
            <span class="text-secondary-400 mr-1 font-semibold"
              >#{{ item.order_no }}</span
            >by
            <span class="text-secondary-400 ml-1 font-semibold">{{
              item.user_name
            }}</span
            ><br />
            {{ item.email }}
        ...
        <template #cell(status)="{ item }">
          <div class="flex justify-end">
            <div
              ...
              :style="{
                background: item.status.bg_color,
                color: item.status.text_color,
              }"
            >
              {{ item.status.stat }}
              <font-awesome-icon
                v-if="item.status.stat === 'on hold'"
                icon="fa fa-ban"
              />
              <font-awesome-icon
                v-else-if="item.status.stat === 'completed'"
                icon="fa fa-check"
              />
            </div>
          </div>
          ...
          <VDropdown placement="bottom-end" :delay="{ show: 300, hide: 100 }">
            ...
            >
              <font-awesome-icon
                ...
                class="xl:text-lg md:text-base text-sm"
              />
            </button>
